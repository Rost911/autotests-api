...
def assert_get_file_with_incorrect_file_id_response(actual: ValidationErrorResponseSchema):
    """
    Проверяет, что API возвращает валидационную ошибку при передаче некорректного UUID в path-параметре file_id.

    :param actual: Фактический ответ API с ошибкой валидации.
    :raises AssertionError: Если структура или значения ошибки отличаются от ожидаемых.
    """
    expected = ValidationErrorResponseSchema(
        details=[
            ValidationErrorSchema(
